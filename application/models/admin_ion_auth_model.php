...
class Admin_ion_auth_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		...
		$this->tables  = $this->config->item('tables', 'admin_ion_auth');

		//initialize data
		$this->identity_column = $this->config->item('identity', 'admin_ion_auth');
		$this->store_salt      = $this->config->item('store_salt', 'admin_ion_auth');
		$this->salt_length     = $this->config->item('salt_length', 'admin_ion_auth');
		$this->join			   = $this->config->item('join', 'admin_ion_auth');
		
		
		//initialize hash method options (Bcrypt)
		$this->hash_method = $this->config->item('hash_method', 'admin_ion_auth');	
		$this->default_rounds = $this->config->item('default_rounds', 'admin_ion_auth');			
		$this->random_rounds = $this->config->item('random_rounds', 'admin_ion_auth');
		$this->min_rounds = $this->config->item('min_rounds', 'admin_ion_auth');				
		$this->max_rounds = $this->config->item('max_rounds', 'admin_ion_auth');	
		

		//initialize messages and error
		$this->messages = array();
		$this->errors = array();
		$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'admin_ion_auth');
		$this->message_end_delimiter   = $this->config->item('message_end_delimiter', 'admin_ion_auth');
		$this->error_start_delimiter   = $this->config->item('error_start_delimiter', 'admin_ion_auth');
		$this->error_end_delimiter     = $this->config->item('error_end_delimiter', 'admin_ion_auth');
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'admin_ion_auth');
		...
		$default_group = $this->where('name', $this->config->item('default_group', 'admin_ion_auth'))->group()->row();
	...
	public function register_with_no_default_group($username, $password, $email, $additional_data = array(), $groups)
	{
		$this->trigger_events('pre_register');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->hash_method = 'sha1' && $user->password === $password || $this->hash_method = 'bcrypt' && $password === true)
			{
                ...
                if ($remember && $this->config->item('remember_users', 'admin_ion_auth'))
                {
	...
	**/
	 ...
	 * @author Ben Edmunds
	 **/
	public function set_lang($lang = 'ar')
	{
		...
		set_cookie(array(
			'name'   => 'lang_code',
			'value'  => $lang,
			'expire' => $this->config->item('user_expire', 'admin_ion_auth') + time()
		));
	...
	private function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			set_cookie(array(
				...
				'expire' => $this->config->item('user_expire', 'admin_ion_auth'),
			));
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->config->item('user_extend_on_login', 'admin_ion_auth'))
			{
				$this->remember_user($user->id);
			}
