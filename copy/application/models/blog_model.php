...
class Blog_model extends CI_Model {
    ...
    public function get($where = NULL, $language = NULL) {
        if(!empty($language)) {
            if ($language == 'arabic' || $language == 'german') {
                $this->db->select('*, titleGE as title, descGE as description');
            } else {
                $this->db->select('*, titleAR as title, descAR as description,');
            }
        } else {
            $this->db->select('*');
        }
    ...
    public function getByTagIdLimited($where = NULL, $limit, $start = 0, $language = NULL) {
        if(!empty($language)) {
            if ($language == 'arabic' || $language == 'german') {
                $this->db->select('blog.*, blog.titleGE as title, blog.descGE as description');
            } else {
                $this->db->select('blog.*, blog.titleAR as title, blog.descAR as description');
            }
        } else {
            $this->db->select('blog.*');
        }
