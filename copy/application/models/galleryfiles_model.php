...
class Galleryfiles_model extends CI_Model {
     ...
     */
    const TABLE_NAME = 'gallery_files';
    ...
    public function getWithLanguage($where = NULL, $language) {
        if(!empty($language) && ($language == 'arabic' || $language == 'german'))
        {
    ...
    public function delete($where = array()) {
       ...
       // echo $this->db->last_query();
        if($query->num_rows() > 0)
        {
          ...
          //print_r($res);
          foreach ($res as $item) {
              $file = 'application/views/images/upload/gallery/'.$item->fileName;
